...
Hmm. Nothing special. Except the message that XOR encryption is used. This Wikipedia article gives some clues about the encryption. The most important property is ![](https://s0.wp.com/latex.php?latex=%28A+%5Cbigoplus+B%29+%5Cbigoplus+B+%3D+A&bg=ffffff&fg=000&s=0)
...
**Step 2:** Let ![](https://s0.wp.com/latex.php?latex=A&bg=ffffff&fg=000&s=0)
 be the key and let ![](https://s0.wp.com/latex.php?latex=B&bg=ffffff&fg=000&s=0)
 be the defaults. Let ![](https://s0.wp.com/latex.php?latex=C&bg=ffffff&fg=000&s=0)
 be the encrypted defaults. We know that ![](https://s0.wp.com/latex.php?latex=C+%3D+A+%5Cbigoplus+B&bg=ffffff&fg=000&s=0)
. In this problem, we have access to the encrypted defaults and to the defaults. Is it possible to obtain the key from this? Yes! Since: ![](https://s0.wp.com/latex.php?latex=A+%3D+%28A+%5Cbigoplus+B%29+%5Cbigoplus+B+%3D+C+%5Cbigoplus+B&bg=ffffff&fg=000&s=0)
. In other words, when applying XOR encryption on the encrypted defaults and the defaults, we get back the key by this property of the XOR encryption.- **Step 3:** We can forge our desired data ([“showpassword” => **“yes”**, “bgcolor” => “#ffffff”]) into the cookie by using the XOR encryption with the found key.
