...
Earlier this year, members of the Python data tools and Impala teams at Cloudera began collaborating to create a new C++ library to eventually become a faster, more memory-efficient replacement for [impyla](https://github.com/cloudera/impyla), [PyHive](https://github.com/dropbox/PyHive), and other (largely pure Python) client libraries for talking to Hive and Impala.

We are excited to release this effort, dubbed hs2client, as a new [Apache-licensed open source project on GitHub](https://github.com/cloudera/hs2client). As you may guess from the name, this library implement the HiveServer2 Thrift API as a C++ library, with careful handling of result sets to allow languages like Python to access data at high performance.
...
Here, I will perform an equivalent fetch using impyla via [Ibis](http://www.ibis-project.org/):
