...
- PageRank
    ...
    - $$In(V_i)$$  是存在指向网页 
    - $$i$$ 的链接的网页集合。
    - $$Out(V_j)$$  是网页 $$j$$ 中的链接指向的网页的集合。
    - {% raw %} |$$Out(V_j)$$| {% endraw %} 是集合中元素的个数。
...
- TextRank
    ...
    - $$w_{ij}$$ 就是是为图中节点 $$V_i$$ 到 $$V_j$$ 的边的权值 。
    - $$d$$ 依然为阻尼系数，代表从图中某一节点指向其他任意节点的概率，一般取值为0.85。
    - $$In(V_i)$$ 和 $$Out(V_i)$$ 也和 PageRank 类似，分别为指向节点 $$V_i$$ 的节点集合和从节点 $V_i$出发的边指向的节点集合。
...
分子是在两个句子中都出现的单词的数量，{% raw %} |$$S_i$$| {% endraw %} 是句子 i 中的单词数。
