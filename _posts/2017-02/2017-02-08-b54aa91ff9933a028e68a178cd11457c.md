...
We benchmarked performance of MKL against the default JVM based execution (referred to as F2JBLAS) and against a popular open source hardware acceleration library called [OpenBLAS](http://www.openblas.net/). For the benchmark, we used an open source suite of performance tests called [*spark-perf*](https://github.com/databricks/spark-perf).
