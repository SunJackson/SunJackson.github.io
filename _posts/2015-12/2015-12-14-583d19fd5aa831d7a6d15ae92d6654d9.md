...
Large-scale time-series data shows up across a variety of domains. In this post, I’ll introduce Time Series for Spark (distributed as the `[spark-ts](https://github.com/sryza/spark-timeseries)` package), a library developed by Cloudera’s Data Science team (and in use by customers) that enables analysis of data sets comprising millions of time series, each with millions of measurements. The package runs atop Apache Spark, and exposes Scala and Python APIs.
...
- A *time series* is a sequence of floating-point values, each linked to a timestamp. In particular, we try as hard as possible to stick with “time series” as meaning a *univariate* time series, although in other contexts it sometimes refers to a series with multiple values at the same timestamp. In Scala, a time series is usually represented by a [Breeze](https://github.com/scalanlp/breeze) vector, and in Python, a 1-D [NumPy](http://www.numpy.org/) array, and has a [DateTimeIndex](https://github.com/sryza/spark-timeseries/blob/master/src/main/scala/com/cloudera/sparkts/DateTimeIndex.scala) somewhere nearby to link its values to points in time.
...
The code, data, and surrounding Maven project for the following example are included in the [spark-ts-examples](https://github.com/sryza/spark-ts-examples) repo.

The most straightforward way to access Spark-TS from Scala is to depend on it in a [Maven](https://maven.apache.org/) project. Do that by including the following repo in the `pom.xml`:
...
Alternatively, to access it in a spark-shell, [download the JAR](https://repository.cloudera.com/cloudera/libs-release-local/com/cloudera/sparkts/sparkts/0.1.0/sparkts-0.1.0-jar-with-dependencies.jar), and then launch the shell with:
...
The central abstraction of the `spark-ts` library is the [TimeSeriesRDD](https://github.com/sryza/spark-timeseries/blob/master/src/main/scala/com/cloudera/sparkts/TimeSeriesRDD.scala), which is simply a collection of time series on which you can operate in a distributed fashion. The time series in the collection are aligned, meaning that position X in the array representing one of the time series refers to the same time point as position X in all the other series. This approach allows you to avoid storing timestamps for each series and instead store a single [`DateTimeIndex`](https://github.com/sryza/spark-timeseries/blob/master/src/main/scala/com/cloudera/sparkts/DateTimeIndex.scala) to which all the series vectors conform.
...
In the layouts introduced above, this is a table of observations. I omitted it here for the sake of brevity, but the `loadTickerObservations` function in [this example](https://github.com/sryza/spark-ts-examples/blob/master/jvm/src/main/scala/com/cloudera/tsexamples/Stocks.scala) loads this into a Spark DataFrame with three columns: “timestamp”, “symbol”, and “price”.  The omitted code is vanilla Spark DataFrames code, nothing specific to`spark-ts` .
...
Finally, perhaps you’re curious about whether the rates of return exhibit serial correlation, which is when the values tend to be related to the values at directly previous time points. One way to figure that is to compute the [Durbin-Watson statistic](https://en.wikipedia.org/wiki/Durbin%E2%80%93Watson_statistic) for each series.
...
**Conclusion**
You should now have good insight into what it feels like to use the `spark-ts` library. If you’re interested in learning more, check out the full [documentation](http://cloudera.github.io/spark-timeseries/0.1.0/index.html).
