...
Follow the Cloudera documentation for installing the [Cloudera Director server](http://www.cloudera.com/documentation/director/latest/topics/director_get_started.html). Although we’ll use AWS as an example here, Cloudera Director can work with Google Cloud Platform and Microsoft Azure as well. Full information on deploying on AWS can be found in our [documentation](http://www.cloudera.com/documentation/director/latest/topics/director_get_started_aws_install_cm_cdh.html), but you’ll want to do the following: 
...
1. Each Instance Group needs a template. Refer to the documentation for assistance in setting that up, but the sparklyr gateway needs a separate template that includes a bootstrap script that sets up sparklyr. Use the script located [here](https://github.com/jordanvolz/director-sparklyr-bootstrap/blob/master/sparklyr-bootstrap.sh) as the bootstrap. You can either upload the file or cut and paste it into the UI.
...
By combining Cloudera Director and sparklyr, R users are able to easily and quickly create environments that leverage Spark and are able to harness the power of big data. For more information on working with sparklyr, visit the [official page](http://spark.rstudio.com/). 

Learn more about Sparklyr in an [upcoming webinar](http://go.cloudera.com/LP=1276) with our partner R Studio
