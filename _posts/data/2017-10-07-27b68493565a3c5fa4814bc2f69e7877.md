...
tags:
    ...
    - giantmastering
    - rh_end_tt_
    - return values deprecating lists
    - algorithms
    - estimated
    - v_end_west
    - pd
...
**This post was originally published as a tutorial for DataCamp [here](https://www.datacamp.com/community/tutorials/networkx-python-graph-tutorial) on September 12 2017** using NetworkX **1.11**. On September 20 2017, NetworkX [announced](https://networkx.github.io/documentation/stable/release/release_2.0) the release of a new
version **2.0**, after two years in the making. While **2.0** introduces lots of great features (some have already been used to improve this project in [postman_problems](https://github.com/brooksandrew/postman_problems)), it also introduced
...
1. This is the NetworkX **2.0** compatible version of the Chinese Postman DataCamp tutorial originally posted [here](https://www.datacamp.com/community/tutorials/networkx-python-graph-tutorial).

1. The ideas introduced in this tutorial are packaged into the [postman_problems](https://github.com/brooksandrew/postman_problems) library which is the mature implementation of these concepts.
...
an [updated notebook](https://github.com/brooksandrew/simpleblog/tree/gh-pages/_ipynb) to a Jekyll flavored markdown document for my blog using [nb2jekyll](https://github.com/jsoma/nb2jekyll) with just a few [tweaks of my own](https://github.com/brooksandrew/simpleblog/tree/gh-pages/nb2jekyll). This was the first Jupyter notebook I’ve converted to a blog post, but
the conversion was smoother than I might have expected. I would recommend [nb2jekyll](https://github.com/jsoma/nb2jekyll) and [this post](http://rjbaxley.com/posts/2017/02/25/Jekyll_Blogging_with_Notebooks.html) to comrade Jekyll bloggers looking to generate posts directly from Jupyter notebooks.
...
This tutorial will first go over the basic building blocks of graphs (nodes, edges, paths, etc) and solve the problem on a real graph (trail network of a state park) using the [NetworkX](https://networkx.github.io/) library in
...
You’ve probably heard of the [Travelling Salesman Problem](https://en.wikipedia.org/wiki/Travelling_salesman_problem) which amounts to finding the shortest route (say, roads) that connects a set of nodes (say, cities). Although lesser known, the Chinese
...
What is a Giantmaster? A [Giantmaster](http://www.sgpa.org/hikes/masters.html) is one (canine or human) who has hiked every trail of Sleeping Giant State Park in Hamden CT (neighbor to my hometown of Wallingford)… in their lifetime. A
Giantmaster Marathoner is one who has hiked all these trails in a single day.

Thanks to the fastidious record keeping of the Sleeping Giant Park Association, the full roster of Giantmasters and their level of Giantmastering can be found [here](http://brooksandrew.github.io/simpleblog). I have to admit this motivated
...
Sleeping Giant Trail Crew (which I was pleased to see recorded in the [SG archive](http://www.sgpa.org/gnews/archive/84.pdf)), new challenges have since arisen. While the 12-month and 4-season Giantmaster categories are impressive and
...
I spent an afternoon annotating these manually by tracing over the image with [GIMP](https://www.gimp.org/):
...
unmarked blazes) which are not required per the [Giantmaster log](http://www.sgpa.org/hikes/MasterLog.pdf), but could be helpful to prevent lengthy double backing. The inclusion of optional trails is actually an established variant of the
CPP called the [Rural Postman Problem](https://en.wikipedia.org/wiki/Route_inspection_problem#Variants). We ignore optional trails in this tutorial and focus on required trails only.
...
to walk up than down. Some metric that combines both distance and elevation change over a directed graph could be incorporated into an extension of the CPP called the [Windy Postman Problem](https://en.wikipedia.org/wiki/Route_inspection_problem#Windy_postman_problem).
...
implementation in the [postman_problems](https://github.com/brooksandrew/postman_problems) package I wrote robustly handles parallel edges in a more elegant way if you’d like to solve the CPP on your own graph with many parallel edges.
...
1. Create a [complete graph](https://en.wikipedia.org/wiki/Complete_graph) connecting every node pair in **1.** with shortest path distance attributes calculated in **2.**

1. Compute a [minimum weight matching](https://en.wikipedia.org/wiki/Matching_(graph_theory)) of the graph calculated in **3.** *(This boils down to determining how to pair the odd nodes such that the sum of the distance between the pairs is as small as possible).*
...
This is the first step that involves some real computation. Luckily `networkx` has a convenient implementation of [Dijkstra’s algorithm](https://en.wikipedia.org/wiki/Dijkstra%27s_algorithm) to compute the shortest path between two nodes. You apply
...
A [complete graph](https://en.wikipedia.org/wiki/Complete_graph) is simply a graph where every node is connected to every other node by a unique edge.
...
A huge thanks to Joris van Rantwijk for writing the orginal implementation on [his blog](http://jorisvr.nl/article/maximum-matching) way back in 2008. I stumbled into the problem a similar way with the same intention as Joris. From Joris’s
2008 post:
...
This Maximum Weight Matching has since been folded into and maintained within the NetworkX package. Another big thanks to the [10+ contributors on GitHub](https://github.com/networkx/networkx/blob/master/networkx/algorithms/matching.py) who have maintained this hefty codebase.
...
one of the most beautiful academic paper titles ever: “Paths, trees, and flowers” [[1](https://cms.math.ca/openaccess/cjm/v17/cjm1965v17.0449-0467.pdf)]. A body of literature has since built upon this work, improving the optimization procedure. The code
implemented in the NetworkX function [max_weight_matching](http://networkx.readthedocs.io/en/networkx-1.10/reference/generated/networkx.algorithms.matching.max_weight_matching.html?highlight=max_weight_matching]) is based on Galil, Zvi (1986) [[2](https://pdfs.semanticscholar.org/6fc3/371dc5d40b638a6b4acb548c8420fa67aac1.pdf)] which employs an O(n3) time algorithm.
...
Now that you have a graph with even degree the hard optimization work is over. As Euler famously postulated in 1736 with the [Seven Bridges of Königsberg](https://en.wikipedia.org/wiki/Seven_Bridges_of_K%C3%B6nigsberg) problem, there exists a path which visits
...
While NetworkX also provides functionality to visualize graphs, they are [notably humble](https://networkx.github.io/documentation/networkx-1.10/reference/drawing.html) in this department:
...
I used [graphviz](http://www.graphviz.org/) and the [dot](https://en.wikipedia.org/wiki/DOT_(graph_description_language)) graph description language to visualize the solution in my Python package [postman_problems](https://github.com/brooksandrew/postman_problems). Although it took some legwork to convert the NetworkX graph structure to
...
If the optimization is any good, these blue lines should represent the least distance possible. Specifically, the minimum distance needed to generate a [matching](https://en.wikipedia.org/wiki/Matching_(graph_theory)) of the odd degree nodes.
...
network fundamentals, you might be interested in Datacamp’s [Network Analysis in Python](https://www.datacamp.com/courses/network-analysis-in-python-part-1) course which provides a more thorough treatment of the core concepts.

Don’t hesitate to check out the [NetworkX documentation](http://networkx.readthedocs.io/en/stable/overview.html) for more on how to create, manipulate and traverse these complex networks. The docs are comprehensive with a good number of [examples](http://networkx.readthedocs.io/en/stable/examples/index.html) and a
series of [tutorials](http://networkx.readthedocs.io/en/stable/tutorial/index.html).

If you’re interested in solving the CPP on your own graph, I’ve packaged the functionality within this tutorial into the [postman_problems](https://github.com/brooksandrew/postman_problems) Python package on Github. You can also piece together the
...
out on the trails on my blog [here](http://brooksandrew.github.io/simpleblog). Another application I plan to explore and write about is incorporating lat/long coordinates to develop (or use) a mechanism to send turn-by-turn directions to my
Garmin watch.
...
[1](https://cms.math.ca/openaccess/cjm/v17/cjm1965v17.0449-0467.pdf): Edmonds, Jack (1965). “Paths, trees, and flowers”. Canad. J. Math. 17: 449–467. [2](https://pdfs.semanticscholar.org/6fc3/371dc5d40b638a6b4acb548c8420fa67aac1.pdf): Galil, Z. (1986). “Efficient algorithms for finding maximum matching in graphs”. ACM Computing Surveys. Vol. 18, No. 1: 23-38.
