...
tags:
    ...
    - visualized
    - spirals
    - following animated visualization
    - funcanimation
    - polar coordinate
    - code
    - colormap
...
The underlying data was released by the Met Office in the United Kingdon, which does excellent work on weather and climate forecasting. The dataset can be downloaded directly [here](https://www.metoffice.gov.uk/hadobs/hadcrut4/data/current/time_series/HadCRUT.4.6.0.0.monthly_ns_avg.txt%60).

The [openclimatedata](https://github.com/openclimatedata) repo on Github contains some helpful data-cleaning code in [this notebook](https://github.com/openclimatedata/climate-spirals/blob/master/data/Climate%20Spirals%20Data.ipynb). You'll need to scroll down to the section titled **Global Temperatures**.
...
To keep our data [tidy](https://cran.r-project.org/web/packages/tidyr/vignettes/tidy-data.html), let's remove rows containing data from 2018 (since it's the only year with data on 3 months, not all 12 months).
...
Lastly, let's compute the mean of the global temperatures from 1850 to 1900 and subtract that value from the entire dataset. To make this easier, we'll create a [multiindex](https://pandas.pydata.org/pandas-docs/stable/advanced.html) using the `year` and `month` columns:
...
We'll start by diving into plotting in a [polar coordinate system](https://en.wikipedia.org/wiki/Polar_coordinate_system).

Most of the plots you've probably seen (bar plots, box plots, scatter plots, etc.) live in the [cartesian coordinate system](https://en.wikipedia.org/wiki/Cartesian_coordinate_system). In this system:
...
Let's dive into how to plot just the data for the year 1850 in matplotlib, then scale up to all years. If you're unfamiliar with creating Figure and Axes objects in matplotlib, I recommend our [Exploratory Data Visualization course](https://www.dataquest.io/course/exploratory-data-visualization).
...
Now, let's tweak the colors. We need the background color within the polar plot to be black, and the color surrounding the polar plot to be gray. We actually used an image editing tool to find the exact black and gray color values, as [hex values](https://en.wikipedia.org/wiki/Web_colors):
...
Right now, the colors feel a bit random and don't correspond to the gradual heating of the climate that the original visualization conveys well. In the original visualiation, the colors transition from blue / purple, to green, to yellow. This color scheme is known as a [sequential colormap](https://matplotlib.org/tutorials/colors/colormaps.html#sequential), because the progression of colors reflects some meaning from the data.

While it's easy to specify a color map when creating a scatter plot in matplotlib (using the `cm` parameter from `Axes.scatter()`, there's no direct parameter to specify a colormap when creating a line plot. [Tony Yu](https://tonysyu.github.io/line-color-cycling.html) has an excellent short post on how to use a colormap when generating scatter plots, which we'll use here.
...
We'll use the [`matplotlib.animation.FuncAnimation`](https://matplotlib.org/api/_as_gen/matplotlib.animation.FuncAnimation.html#matplotlib.animation.FuncAnimation) function to help us with this. To take advantage of this function, we need to write code that:
...
All that's left now is to re-format our previous code and add it to the skeleton above. We encourage you to do this on your own, to practice programming using matplotlib. [Here's](https://www.dataquest.io/blog/large_files/climate_spiral.gif) what the final animation looks like in lower resolution (to decrease loading time).
